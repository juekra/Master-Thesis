...
public class FindMax_Parallel4 implements JavaDelegate {

	@Override
	public void execute(DelegateExecution execution) throws Exception {


        int counter = ProcessInstanceData_Parallel4.count();
        int startListSize = ProcessInstanceData_Parallel4.startList.size();
        int threadAmount = ProcessInstanceData_Parallel4.threadAmount;
        if(!(counter>=threadAmount)){
            ...
            List<Integer> startList = ProcessInstanceData_Parallel4.startList;

            System.out.println("Thread-Nr:" + counter + " searching from " + startIndex + " to " + endIndex);

            int max = -1;
            int prime = 0;
            for(int repeatCounter = 0; repeatCounter<ProcessInstanceData_Parallel4.repetitions; repeatCounter++){
            ...
            ProcessInstanceData_Parallel4.addIntermediateResult(max);
        }else{
