...
public class FindMax_Parallel2 implements JavaDelegate {

	@Override
	public void execute(DelegateExecution execution) throws Exception {


        int counter = ProcessInstanceData_Parallel2.count();
        int startListSize = ProcessInstanceData_Parallel2.startList.size();
        int threadAmount = ProcessInstanceData_Parallel2.threadAmount;
        if(!(counter>=threadAmount)){
            ...
            List<Integer> startList = ProcessInstanceData_Parallel2.startList;

            System.out.println("Thread-Nr:" + counter + " searching from " + startIndex + " to " + endIndex);

            int max = -1;
            int prime = 0;
            for(int repeatCounter = 0; repeatCounter<ProcessInstanceData_Parallel2.repetitions; repeatCounter++){
            ...
            ProcessInstanceData_Parallel2.addIntermediateResult(max);
        }else{
